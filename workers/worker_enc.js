...
// Server-side: Decrypt Base64 RSA payload using PKCS8 Private Key
async function decryptCredential(encryptedB64, privateKeyPem) {
    ...
    const encryptedBuffer = Uint8Array.from(atob(encryptedB64), c => c.charCodeAt(0)).buffer;
...
export default {
    async fetch(request, env, ctx) {
      ...
      <body>
          ...
          <script>
              // Paste your RSA Public Key PEM below
              const PUBLIC_KEY_PEM = \`-----BEGIN PUBLIC KEY-----
MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEAu1P...
-----END PUBLIC KEY-----\`;
              ...
              tokenInput.value = localStorage.getItem('resume_access_token_plain') || '';
              ...
              tokenInput.addEventListener('input', async () => {
                  const plainVal = tokenInput.value.trim();
                  localStorage.setItem('resume_access_token_plain', plainVal);
                  if (plainVal) {
                      const encrypted = await encryptCredential(plainVal);
                      localStorage.setItem('resume_access_token_encrypted', encrypted);
                  } else {
                      localStorage.removeItem('resume_access_token_encrypted');
                  }
              });
              ...
              document.getElementById('resumeForm').addEventListener('submit', async (e) => {
                  ...
                  let encryptedToken = localStorage.getItem('resume_access_token_encrypted') || '';
                  if (!encryptedToken && tokenInput.value.trim()) {
                      encryptedToken = await encryptCredential(tokenInput.value.trim());
                      localStorage.setItem('resume_access_token_encrypted', encryptedToken);
                  }
        ...
        if (request.method === "POST" && url.pathname === "/generate") {
            try {
                const { applicant, jd, encryptedAccessToken } = await request.json();

                let decryptedToken = null;
                if (encryptedAccessToken && env.RSA_PRIVATE_KEY) {
                    decryptedToken = await decryptCredential(encryptedAccessToken, env.RSA_PRIVATE_KEY);
                }

                const apiKey = decryptedToken || env.GEMINI_API_KEY;
